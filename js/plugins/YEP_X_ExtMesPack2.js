...
//=============================================================================
 /*:
 * @plugindesc v1.00 拓展信息包 2(需要 YEP_MessageCore.js)
 ...
 *
 * 这个插件利用文本代码向玩家传递重要信息。
 * 它可以将玩家拥有的物品、武器和盔甲的数量传递给各种参数、
 * 额外参数以及角色和敌人的特殊参数。
 * 新的文本代码还允许您基于两个数字比较来更改文本颜色，
 * 以便有条件地为文本着色。
 * 除了条件颜色之外，
 * 还可以根据开关值或自定义条件显示文本。
 *
 * 注意:这个插件最适合RPG Maker MV 1.5.0+。
 * RPG Maker MV的较低版本仍然可以使用该插件，
 * 但是您将无法舒适地充分利用插件参数功能。
 *
 * ============================================================================
 * 文本代码
 ...
 * ============================================================================
 *
 * 对于那些有JavaScript经验的人来说，如果想定制这个
 * 插件提供的文本代码的行为方式，你可以在插件参数中
 * 插件提供的文本代码的行为方式，你可以在插件参数中
 *
 * 插件参数内存在游戏内消息功能转换每个文本代码时使
 * 用的代码。参考每个代码顶部注释中显示的变量，
 * 以了解正在使用哪些变量以及它们是如何使用的。
 * ...
 *
 * 默认情况下:
 *
 *   x
 *   - 指插入文本代码的x变量。这可以是一个数字或字符串，
 *   具体取决于文本代码。
 *
 *   y
 *   - 指插入文本代码的y变量。这可以是许多字符串，
 *   具体取决于文本代码。
 *
 *   text
 *   - 指将由信息系统显示的文本。
